...
bounds2df <- function(bounds.conf.int, theta.obs, transform=TRUE, time.zero=TRUE){
...
.get.uniform.RV <- function(theta.obs, psi, tau,
  ...
  if (pvalue < 1-conf.level) {
    ...
    .get.pvalue.sens <- function(x){
      ...
      # if (lower.b) {
      #     # test statistics
      #     test.stat.sens <- n^(1/2)*sum(theta.obs-sens.all*(sqrt(psi)*sqrt(tau))) # can add weight here
      #     IF.vals.effect.lower <- IF.vals.theta.obs - inner.func.2
      #     epsilon <- .estimate.limit.dist(IF.vals = IF.vals.effect.lower)
      # } else {
      #     # test statistics
      #     test.stat.sens <- n^(1/2)*sum(theta.obs+sens.all*(sqrt(psi)*sqrt(tau))) # can add weight here
      #     IF.vals.effect.upper <- IF.vals.theta.obs + inner.func.2
      #     epsilon <- .estimate.limit.dist(IF.vals = IF.vals.effect.upper)
      # }
      # dist.null.sens <- rowSums(epsilon)
      # p-value
